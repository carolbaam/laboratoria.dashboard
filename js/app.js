...
for (var i = 0; i < optionMenu.length; i++) {
  optionMenu[i].addEventListener("click", showMenu);
}

function showMenu() {
  var activeMenu = this.getElementsByClassName('sub_menu')[0];
...
/* Creando animaciones del tab */

/*var hideShow = function(e) {
  var tabSeleccionado = e.target.dataset.tabSeleccionado;
  var showOverview = document.getElementById('overview');
  var showStudents = document.getElementById('students');
  var showTeachers = document.getElementById('teachers');
...
function onCharge() {
  ...
  for (var i = 0; i < tabElements.length; i++) {
    tabElements[i].addEventListener("click", hideShow)
  }
}
